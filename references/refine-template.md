...
- The Figma MCP — Figma is the source of truth. Local Dev Mode MCP is read-only; ensure the Figma file is open in Figma desktop while this command runs. Use `mcp__figma-local__get_variable_defs` to pull the file's variable values when reasoning numerically about tokens.
...
1. Pull the Figma node's measured properties via `mcp__figma-local__get_design_context` and `mcp__figma-local__get_variable_defs`. Record the relevant numbers: width / height, padding/margin per side, gap, font-family, font-size (computed in px), font-weight, line-height, letter-spacing, color values (resolved from variables). For tokens, record both the token name and its concrete value at the matching mode.
