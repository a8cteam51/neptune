...
7. If any change was applied under `wordpress/wp-content/themes/<themeSlug>/` during this run (template / part edit, `theme.json` edit, block stylesheet edit, `functions.php` edit), invoke the `theme-validator` subagent (Task tool, `subagent_type: theme-validator`) to catch cross-file regressions the chunk-level validator and screenshot diff cannot see — broken `template-part` slug references, unresolved preset references, missing `templateParts` registrations, required-block omissions. Fix any `ERROR` rows it returns before declaring the refinement done; surface `WARNING` rows in the run summary.
