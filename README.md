...
5. Generates `theme.json` and lifts reusable Figma components into a library of named WordPress block patterns.
6. Builds and refines each template using validated block markup, `theme.json`, registered patterns, and optional block stylesheets.

Block markup is validated against a real WordPress site through `mcp__wordpress-studio__validate_blocks` before being written to disk — Studio MCP runs validation against the actual WordPress install the markup will render in, so the validation context matches the rendered context.
...
| 6    | `extract-patterns` skill             | Walk the Figma file's published components and emit each one as a validated WordPress block pattern under `patterns/`. Record the slug-to-component mapping in `neptune-config.json` so `/build-*` references patterns by slug instead of re-emitting markup. Final onboarding step. |
...
- `figmaVariables` — file-scoped variable snapshot pulled by `pull-figma` via a `use_figma` JS call (variables, their owning collections, and resolved values per mode). Drives `theme-json`'s palette / typography / spacing emission.
...
- `devNotes` — every `💬 Dev Note` instance on the dev-handoff page, captured by `pull-figma` via a single `use_figma` JS call. Keys are stable, context-derived identifiers (e.g. `header-logo-spacing`, `front-page-cta-overlap`); values are `{text, context}` where `context` names the parent layout and any element the note overlaps or its connector points at.
...
- `patterns` — registry of WordPress block patterns lifted from Figma's published components by `extract-patterns`. Keys are pattern slugs; each value carries `{title, fullSlug, figmaComponentId, figmaComponentKey, file}` so `/build-template` and `/build-content` can reference patterns by slug (`<!-- wp:pattern {"slug":"<themeSlug>/<pattern-slug>"} /-->`) instead of re-emitting their markup.
- `patternsCompleted` (boolean) — set by `extract-patterns` once the run finishes (also set when the user explicitly opts out of extraction, since the flag signals the workflow has moved past this phase).
...
- **Stage A (numeric).** Pull the Figma node's measured properties via `mcp__figma__get_design_context`, and pull the file's variable definitions via `use_figma` (load the `figma:figma-use` skill first). Compare against the rendered DOM's computed styles (via `theme.json` declarations, block stylesheets, and serialised block `style=` attributes; via `studio wp_cli` introspection where browser-side measurement isn't reliably available). Anything outside ±1px on layout, ±2% perceptual on color, or any difference in `font-weight` / `font-family` / token name is a discrepancy.
