...
export function parseAgentJson(input: string, label: string): unknown {
	try {
		...
		// wrapping the envelope in prose ("Looking at the post-content body…")
		...
		// always far bigger than any prose-embedded JSON snippet (e.g. an
		// echoed `{"slug":"foo"}` reference). Falls through to the original
		// strict-parse error when no candidate parses.
