...
export default function RefineTemplate({activeProject, onDone}: Props) {
	...
	const beginRun = (pull: PickablePull) => {
		...
		(async () => {
			try {
				const result = await runRefine(activeProject, pull, ev =>
...
async function runRefine(
	...
	const studio = await openStudioSession();
	...
	await writeFile(renderedPath, renderedBuf);
	onEvent({
		kind: 'step',
	...
	if (outcome.ok) {
		await writeFile(diffPath, outcome.diff.pngBuffer);
	...
	if (!refined.trimStart().startsWith('<!--')) {
		throw new Error(
...
function buildRefineContent(inputs: RefineInputs): ContentBlock[] {
	...
	content.push(textBlock(
		`Refine this Gutenberg block markup template (${inputs.templateFile}) so its rendered output matches the design more closely. Output raw block markup only, per the refine-blocks skill.`,
	));
...
function previewPathFor(templateFile: string): string {
	...
	if (stem === '404') return '/this-path-should-404-' + Date.now();
