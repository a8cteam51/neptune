...
description: Map Figma `🗒️ Templates` title cards to WordPress block-theme files, confirm each mapping with the user, scaffold the empty template files, capture the Figma node IDs for each desktop/mobile layout, and capture the page URL where each template renders so later build/refine/content commands and header/footer nav wiring can reuse them without re-asking. Use after dev-notes; the `theme-json` skill (run later in the flow) registers the resulting parts and custom templates in `theme.json`.
---
...
6. Create each mapped template file empty, in the correct directory under the theme (use `themeSlug` from `neptune-config.json`). Do not populate the files with block markup yet — that happens in `/build-template`. Do not write to `theme.json` from this skill; the `theme-json` skill (run later in the flow, after `download-assets`) reads `templateMappings` from `neptune-config.json` and registers the resulting template parts and custom templates there.
...
8. Tell the user mappings have been recorded and the next skill in the flow is `download-assets`, which will export every image asset from the desktop layouts and import them into the WP media library. After that, `theme-json` will generate `theme.json` and register the template parts and custom templates from `templateMappings`.

9. Once completed, move onto the `download-assets` skill if `neptune-config.json` does not already contain a `figmaAssets` field. If it does, ask the user whether to run `download-assets` now or skip it (they can always run it later if they skip).
