...
3. Find the `🎨 Style Guide` layer in the Figma file. Cross-reference its visual base styles against the variable tables to build the style dataset (colors, typography, spacing, etc.) that theme.json needs.
...
5. Tell the user `theme.json` has been written and that the next skill in the flow is `map-design-templates`.
