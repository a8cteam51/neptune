...
7. Tell the user `theme.json` has been written and that the next step is the `/build-template <name>` slash command (run once per unique `wordpressFile` in `templateMappings`).
