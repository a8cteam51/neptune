...
Fill the body content for every `templateMappings` entry in `neptune-config.json` whose target WP post is still empty (or carries only the WP default stub). `$ARGUMENTS` may contain an optional `--skip=<comma-separated-names>` flag to exclude specific mappings from this run (matched against `templateMappings` keys, e.g. `--skip=Blog,404 Page`). Treat `--skip` as optional — if it's absent, do not prompt for it. There is no base-site-URL argument here: each entry's `pageUrl` is the source of truth for which post to fill, and `pageUrl` was captured during `/map-design-templates`.

This command is a batch wrapper around `${CLAUDE_PLUGIN_ROOT}/commands/build-content.md`. Read that file first — every guardrail there (no `wp:html` fallback, block markup only, `register_block_style` workflow, GitHub issues for human follow-ups, internal-link wiring from `templateMappings`) applies to each entry filled in this run.
...
6. For each entry to fill, in `templateMappings` key order, follow the procedure in `${CLAUDE_PLUGIN_ROOT}/commands/build-content.md` from step 4 onward (steps 1–3 there are covered by this run's steps 1, 3, and 4): pull the Figma design context for the entry's `figmaNodes`, apply `devNotes`, generate body block markup via `wp-blockmarkup`, write to a temp file, and update the post with `studio wp post update <id> --post_content="$(cat <tmpfile>)"`. Verify and clean up the temp file as in `/build-content` step 8. Treat each entry as an independent invocation: do not carry block markup or assumptions from one entry into another beyond what's already shared via `theme.json` and registered block styles.

7. After all entries are processed, output a summary: filled entries (entry key → post ID → `pageUrl`), skipped (with reason — `--skip`, missing `figmaNodes`, no `pageUrl`, post not found, already filled), and any GitHub issues opened during the run. Remind the user that `/refine-template <name> <page-url>` is the next step for any newly filled page where they want a visual diff against Figma — `/build-all-content` does not auto-refine because refinement requires a rendered screenshot and the user may want to batch refinement separately via `/refine-all-templates`.
