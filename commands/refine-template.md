...
allowed-tools: Read, Edit, Write, Glob, Grep, Bash(gh issue create:*), Bash(gh repo view:*), Bash(npm run build:styles:block-styles), Bash(studio wp:*), Bash(curl:*), Bash(${CLAUDE_PLUGIN_ROOT}/scripts/check-state.sh:*), Skill, mcp__figma__*, mcp__wordpress-studio__*
---
...
- The Figma MCP — Figma is the source of truth. Load the `figma:figma-use` skill before any Figma MCP calls that need JS execution in the file context. Use `mcp__figma__get_variable_defs` to pull the file's variable values when reasoning numerically about tokens.
...
LLM vision is unreliable for sub-10px spacing deltas and for telling apart neighbouring font-weights. This command therefore runs the diff in two stages — numeric measurement first, vision second — and trusts numeric measurements over visual impressions when they conflict.

**Stage A (numeric).** For each region under refinement:

1. Pull the Figma node's measured properties via `mcp__figma__get_design_context` and `mcp__figma__get_variable_defs`. Record the relevant numbers: width / height, padding/margin per side, gap, font-family, font-size (computed in px), font-weight, line-height, letter-spacing, color values (resolved from variables). For tokens, record both the token name and its concrete value at the matching mode.
...
1. Take a screenshot of the rendered template at the user-provided site URL via `mcp__wordpress-studio__take_screenshot`. Take one screenshot per breakpoint you intend to refine against (desktop and mobile widths) — `take_screenshot` accepts a viewport argument. Record which breakpoint each screenshot represents.
