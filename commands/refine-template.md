...
5. Apply refinements to resolve each discrepancy — edits to block markup, `theme.json`, or block stylesheets as appropriate. When a discrepancy involves an image (wrong `src`, missing `id`, wrong alt, missing image entirely), look the image up in `figmaAssets` from `neptune-config.json` — match by Figma node ID against `figmaAssets.<hash>.figmaNodeIds` or by image hash directly — and set the `<!-- wp:image -->` (or `wp:cover` / `wp:media-text`) attributes from that entry's `wpAttachmentId`, `wpUrl`, and `altText`. If the required image is not in `figmaAssets`, stop and tell the user to (re-)run `download-assets`; never invent placeholder image URLs or embed Figma URLs directly.
